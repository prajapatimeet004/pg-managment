...
export function Reports() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        revByMonth[6].revenue = transactions.reduce((acc: number, t: any) => acc + t.amount, 0);
        ...
        const propRev = properties.map((p: any) => {
          const rev = transactions
            .filter((t: any) => t.property_name === p.name)
        ...
        const compCat = categories.map(c => ({
          category: c,
          count: complaints.filter((co: any) => co.category === c).length
        }));
        ...
        setData({
          ...
          occupancy: [
            ...
            { month: "Apr", rate: properties.reduce((acc: number, p: any) => acc + (p.occupied_beds / p.total_beds) * 100, 0) / properties.length }
          ],
          propertyRevenue: propRev,
          complaints: compCat,
          metrics: {
            total_revenue: revByMonth[6].revenue,
            occupancy_rate: properties.reduce((acc: number, p: any) => acc + (p.occupied_beds / p.total_beds) * 100, 0) / properties.length,
            collection_rate: (tenants.filter((t: any) => t.rent_status === "paid").length / tenants.length) * 100 || 0,
            avg_resolution: "18h"
          },
          aiInsight: insight
        });
  ...
  const topProperty = data.propertyRevenue.sort((a: any, b: any) => b.value - a.value)[0];
  return (
              ...
              <ResponsiveContainer width="100%" height={350}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {data.propertyRevenue.map((_entry: any, index: number) => (
